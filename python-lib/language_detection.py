...
class languageDetection:
    ...
    def _confidence_level_check(self, lang_id, lang_probability):
        ...
        if self.params['confidence_level_required'] and lang_probability < self.params['confidence_level']/100.:
            lang_id = self.params['fallback_output']
            lang_name = self._get_language_name_from_id(self.params['fallback_output'])
            return self._to_pdSeries(lang_id, 
                                     lang_name, 
                                     lang_probability)
        
        else:
            return self._to_pdSeries(lang_id, 
                                     self._get_language_name_from_id(lang_id), 
                                     lang_probability)
    ...
    def _to_pdSeries(self, lang_id, lang_name, lang_probability):
        ...
        return pd.Series({self.col + '_lang_id': lang_id,
